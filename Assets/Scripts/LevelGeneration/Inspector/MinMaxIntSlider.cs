...
namespace Assets.Scripts.LevelGeneration.Test2.Editor
{
    public class MinMaxIntSliderAttribute : PropertyAttribute
    {
        ...
        public MinMaxIntSliderAttribute(float min, float max)
        {
            this.min = min;
            this.max = max;
        }
    ...
    public class MinMaxSliderDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var minMaxAttribute = (MinMaxIntSliderAttribute)attribute;
            ...
            if (propertyType == SerializedPropertyType.Vector2)
            {
                ...
                Vector2 vector = property.vector2IntValue;
                float minVal = vector.x;
                float maxVal = vector.y;

                //F2 limits the float to two decimal places (0.00).
                minVal = EditorGUI.FloatField(splittedRect[0], int.Parse(minVal.ToString("F2")));
                maxVal = EditorGUI.FloatField(splittedRect[2], int.Parse(maxVal.ToString("F2")));
