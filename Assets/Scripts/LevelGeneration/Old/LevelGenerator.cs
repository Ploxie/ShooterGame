...
public class LevelGenerator : MonoBehaviour
{
    ...
    [SerializeField] private Room module;
    ...
    private void OnValidate()
    {
        ...
        Vector2Int start = new Vector2Int((int)(transform.position.x / level.TileSize), (int)(transform.position.y / level.TileSize));

        RandomWalk(start, stepCount);

    }

    private void RandomWalk(Vector2Int start, int steps)
    {
        if (steps <= 0)
            return;

        TileData previousTile = new TileData(start);

        level.AddTile(new TileData(start));

        for (int i = 0; i < steps - 1; i++)
        {
            Vector2Int direction = GenerateValidRandomDirection(previousTile.Position);

            Vector2Int position = previousTile.Position + direction;
            TileData tile = new TileData(position, previousTile);
            level.AddTile(tile);
    ...
    private Vector2Int GenerateValidRandomDirection(Vector2Int from)
    {
    ...
    /*void OnValidate() // Should be onStart
    {
        Random.InitState(seed);
        ...
        {
            tile.Position = position + direction + tile.Position;
            tiles.Add(tile.Position, tile);
        }
        Tile closest = room.FindClosestTile(direction * 10).Tile;
        closest.AddNeighbour(previousTile);
        previousTile.AddNeighbour(new Tile(closest.Position, tileSize));
        /*List<RoomTile> edges = room.FindEdgeTiles();
        Vector2 edgePosition = room.FindEdgeTiles()[Random.Range(0, edges.Count)].Tile.Position;
        previousTile = tiles[edgePosition];
        ...
        for (int i = 0; i < branchLength; ++i)
        {
            ...
            if(i == 3)
            {
                ...
                foreach(Tile tile in room.Tiles)
                {
                    ...
                    tiles.Add(position + tile.Position, tile);
                }
                RoomTile closest = room.FindClosestTile(-direction * 10);
                previousTile.AddNeighbour(new Tile(position + closest.Tile.Position, tileSize));
                List<RoomTile> edges = room.FindEdgeTiles();
                Vector2 edgePosition = room.FindEdgeTiles()[Random.Range(0, edges.Count)].Tile.Position;
                previousTile = tiles[edgePosition];
            }
            else
            {
                ...
                // chance to spawn module
                // lookup module
                // spawn instead of tile
    ...
    private void GenerateMeshes()
    {
        ...
        foreach (Wall door in doors)
        {
            ...
            door.DoorPosition =  b.HasDoors(mask) ? Random.Range(0.25f, 0.75f) : 0.5f;
            door.DoorSize =  b.HasDoors(mask) ? Random.Range(0.25f, 1.0f) : b.HasNeighbours(mask) ? 1.0f : 0.0f;
    ...
    private void OnDrawGizmos()
    {
        ...
        foreach(TileData tile in level.Tiles.Values)
        {
            Vector3 tileWorldPosition = new Vector3(tile.Position.x, 0, tile.Position.y) * level.TileSize;
